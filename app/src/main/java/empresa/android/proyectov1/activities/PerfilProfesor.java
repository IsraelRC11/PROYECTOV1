...
public class PerfilProfesor extends BaseActivity {

    private ImageView ivPerfil, btnEditarDatos;
    private TextView tvNombre;
    private MaterialButton btnLogout, btnEspecialidades, btnEditPass;
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    if (urlFoto != null && !urlFoto.isEmpty()) {
                        ivPerfil.setPadding(0, 0, 0, 0);
                        ivPerfil.setImageTintList(null);
                        Glide.with(PerfilProfesor.this).load(urlFoto).placeholder(R.drawable.usuario).into(ivPerfil);
                    }
                }
            }
            @Override
    ...
    private void mostrarDialogoEditarPerfil() {
        ...
        btnGuardar.setOnClickListener(v -> {
            ...
            if (nuevoNombre.isEmpty() || nuevoApellido.isEmpty()) {
                Toast.makeText(PerfilProfesor.this, "Por favor, completa todos los campos", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            mDatabase.child("Usuarios").child(uid).updateChildren(actualizacionesPerfil)
                    .addOnSuccessListener(aVoid -> {
                        Toast.makeText(PerfilProfesor.this, "Perfil actualizado correctamente", Toast.LENGTH_SHORT).show();
                        dialog.dismiss();
                    })
                    .addOnFailureListener(e -> Toast.makeText(PerfilProfesor.this, "Error al actualizar", Toast.LENGTH_SHORT).show());
        });
    ...
    private void mostrarDialogoCambiarContrasena() {
        ...
        btnGuardar.setOnClickListener(v -> {
            ...
            if (nuevaContra.isEmpty() || confirmarContra.isEmpty()) {
                Toast.makeText(PerfilProfesor.this, "Completa todos los campos", Toast.LENGTH_SHORT).show();
                return;
            }

            if (nuevaContra.length() < 6) {
                Toast.makeText(PerfilProfesor.this, "La contraseña debe tener al menos 6 caracteres", Toast.LENGTH_SHORT).show();
                return;
            }

            if (!nuevaContra.equals(confirmarContra)) {
                Toast.makeText(PerfilProfesor.this, "Las contraseñas no coinciden", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            if (user != null) {
                ...
                user.updatePassword(nuevaContra).addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        Toast.makeText(PerfilProfesor.this, "Contraseña actualizada en Firebase", Toast.LENGTH_SHORT).show();
                        dialog.dismiss();
                    } else {
                        btnGuardar.setEnabled(true);
                        Toast.makeText(PerfilProfesor.this, "Error: Vuelve a iniciar sesión para cambiarla", Toast.LENGTH_LONG).show();
                    }
                });
            }
        });
